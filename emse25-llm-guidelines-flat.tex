...
G5 & \ref*{sec:use-suitable-baselines-benchmarks-and-metrics} & \hyperref[sec:use-suitable-baselines-benchmarks-and-metrics]{Use Suitable Baselines, Benchmarks, and Metrics} \\
G6 & \ref*{sec:use-an-open-llm-as-a-baseline} & \hyperref[sec:use-an-open-llm-as-a-baseline]{Use an Open LLM as a Baseline} \\
G7 & \ref*{sec:use-human-validation-for-llm-outputs} & \hyperref[sec:use-human-validation-for-llm-outputs]{Use Human Validation for LLM Outputs} \\
...
\vspace{0.5\baselineskip}
\begin{framed}
\tldr: Researchers \must transparently report study limitations, including the impact of non-determinism and generalizability constraints. The \paper~\must specify whether generalization across LLMs or across time was assessed, and discuss model and version differences. Authors \must describe measurement constructs and methods, disclose any data leakage risks, and avoid leaking evaluation data into LLM improvement pipelines. For studies involving sensitive data, they \must discuss data governance mechanisms. They \should justify LLM usage in light of its resource demands. Mitigation strategies such as replication packages, human validation, longitudinal re-runs, triangulation, and sensitivity analysis \should be employed and reported where applicable. Where full data sharing is not possible, a subset of the validation data \should be included to enable partial replication. \end{framed}
...
\begin{itemize}[label=$\rightarrow$]
    ...
    \item Section~\design: architecture-level triangulation, prompt design, and sensitivity analysis.
    \item Section~\traces: audit trails and longitudinal re-runs.
...
% G5 \iconS line: G5 Recommendations - "researchers \should repeat experiments and report descriptive statistics".
G5 & Meaningful evaluation requires reasoned valid measurement. &
...
% G7 \iconS line: G7 Subjective Judgment and Agreement - "the LLM output \should be evaluated against judgments aggregated from multiple human judges"; "Researchers \should report measures of IRA or IRR".
G7 & Automated metrics alone cannot ensure validity of subjective constructs. &
