...
\begin{itemize}[label=$\square$]
    ...
    \item \iconM Report the list of tools and skills exposed to the model (names and purposes) in the \paper~(G3).
    \item \iconM Describe prompt development strategies (e.g., zero-shot, few-shot), rationale, and selection process in the \paper~(G3).
    \item \iconM Document input handling and token optimization strategies when prompts are long or complex~(G3).
    \item \iconM Report generation and collection processes for dynamically generated or user-authored prompts~(G3).
    ...
    \item \iconS Include complete tool schemas, skill definitions, sub-agent definitions, and connected MCP servers as \supplementarymaterial~(G3).
