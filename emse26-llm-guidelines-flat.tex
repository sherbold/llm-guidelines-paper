...
This guideline applies to all study types, although the need for human validation varies. The Replacing Human Judgment recommendation applies particularly to \annotators, \judges, \synthesis, and \subjects, where the LLM substitutes for human input.
...
% Human Validation \iconS replacement line: Recommendations (Replacing Human Judgment) - "researchers \should follow systematic approaches to decide whether and how the replacement is justified ... such as Krippendorff's $\alpha>0.8$"; "researchers \should additionally validate a sample against human experts"; "Researchers \should justify why the LLM is methodologically appropriate".
...
\begin{itemize}[label={},leftmargin=*]
    ...
    \item \iconS When LLMs replace, rather than augment, humans in research tasks, report in the \paper the systematic approach used to decide that the replacement is justified, including model-to-model and model-to-human agreement, and for qualitative coding of interpretive data, justify why an LLM is methodologically appropriate~(\refhumanvalidation).
    % G7 Recommendations (Replacing Human Judgment): "researchers \should follow systematic approaches to decide whether and how the replacement is justified, e.g., showing that a jury of three LLMs exhibits model-to-model agreement... such as Krippendorff's $\alpha>0.8$"; "researchers \should additionally validate a sample against human experts"; "Researchers \should justify why the LLM is methodologically appropriate".
