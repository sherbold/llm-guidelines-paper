...
Beyond Reviewer 1's points, we took the opportunity to incorporate further input from discussions during ICSE 2026 (April 2026) and the 3rd Copenhagen Symposium on Human-Centered Software Engineering AI (May 2026), and to strengthen the construct-validity guidance for benchmarks based on two recent surveys (Cao et al.'s review of 572 benchmarks and Bean et al.'s review of 445 benchmarks). The substantive additions include new \textbf{should} recommendations for benchmarks (defining the phenomenon, justifying the sampling strategy, isolating confounders, error analysis, contamination prevention) and human validation (rater demographics for value-laden constructs); a new \textbf{should} on recording runtime traces in an OTLP-compatible format such as the OpenTelemetry GenAI semantic conventions or OpenInference; bracketed conditional tags (e.g., \texttt{[fine-tuning]}, \texttt{[agents]}) prefixing applicability-conditioned items in the reporting checklist; and a bibliography pass that upgraded seven arXiv preprints to their peer-reviewed venues. The companion website now hosts a changelog documenting all changes between guideline versions, available at \url{https://llm-guidelines.org/changelog/}.
\end{response}
...
On the suggestion to use \emph{mandatory} and \emph{desirable} in Table~3, we hope the reviewer agrees with us keeping \textbf{must} and \textbf{should}, for the same reason we gave in the previous round: after dropping the RFC reference, we retained \textbf{must}/\textbf{should} as plain indicators of essential vs.\ desired practices, in the same spirit as the SIGSOFT Empirical Standards' \emph{mandatory}/\emph{desirable} categories.
The categorical distinction is also carried visually in Table~3 by the filled (\textbullet) and open ($\circ$) icons defined in the caption, so the table conveys the same essential/desired split that the SIGSOFT Empirical Standards' terms encode.
Using a single vocabulary across Table~3, the guideline recommendations, and the reporting checklist (Appendix~C) lets readers map matrix cells directly to the corresponding sentences in the body, which we believe is more useful than aligning Table~3 with the SIGSOFT Empirical Standards' terminology while the surrounding prose uses different words.
\end{response}
...
Each item is now introduced directly by its \textbf{must}~(\textbullet) or \textbf{should}~($\circ$) icon, which serves as the bullet figure, matching the symbology of Table~3.
\end{response}
