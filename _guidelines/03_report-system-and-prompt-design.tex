...
\textbf{Where to report.} Throughout this section, the \paper \must contain a high-level description of any reported component, with full details provided as \supplementarymaterial. Standalone LLMs require limited architectural documentation, while study setups that integrate LLMs with other components, configure agents, or depend on particular context files or tool schemas require more detailed reporting.
...
If an LLM is used as a \emph{standalone system}, for example by sending prompts directly to a GPT-4o model via the OpenAI API without pre-processing the prompts or post-processing the responses, a brief explanation is usually sufficient.
