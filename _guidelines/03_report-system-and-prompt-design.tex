...
Researchers \must describe which context-file mechanisms were used in their setup and \should publish the full contents (snapshot at study time) as \supplementarymaterial, because they shape agent behavior similarly to system prompts. Mid-session edits to context files (e.g., when agents update \texttt{CLAUDE.md} during a run) are tool calls and are reported under \traces.
