...
Claude Code, for example, does not register individual command-line utilities; it instead defines a single \texttt{Bash} tool (\texttt{PowerShell} on Windows) through which the agent runs arbitrary shell commands such as \texttt{grep} or \texttt{ls}.\footnote{\url{https://code.claude.com/docs/en/tools-reference}}
